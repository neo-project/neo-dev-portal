...
The [Safe] attribute means that this method does not modify the contract data and is safe to access.
...
`ContractAuthor` represents the author field, which can be filled with the author's name and email address. It will output to the extra json object in manifest.json.

`ContractEmail` represents the email field. It will output to the extra json object in manifest.json.

`ContractSourceCode` represents the URL of the contract source code. It will output to the extra json object in manifest.json.

`ContractVersion` represents the version of the contract. It will output to the extra json object in manifest.json.

`ContractDescription` represents the description of the contract. It will output to the extra json object in manifest.json.

`ManifestExtra` represents the extra fields in the Manifest file, where you can add `WebSite`, `Docs` and etc. It will output to the extra json object in manifest.json.
